...
% Aplicar preénfasis a las señales
tau = 75e-6; % Constante de tiempo del preénfasis (75 microsegundos)
preemphasis_filter = designfilt('highpassiir', 'FilterOrder', 1, ...
    'HalfPowerFrequency', 1/(2 * pi * tau), 'SampleRate', fs);
fxRight_preemphasized = filter(preemphasis_filter, fxRight);
fxLeft_preemphasized = filter(preemphasis_filter, fxLeft);
fxCenter_preemphasized = filter(preemphasis_filter, fxCenter);
...
desCH01 = sin(2 * pi * (fxRight_preemphasized + fxbaja) .* t); % Multiplicación element-wise
desCH02 = sin(2 * pi * (fxLeft_preemphasized + fxmedia) .* t);
desCH03 = sin(2 * pi * (fxCenter_preemphasized + fxalta) .* t);
...
demodulated_signal_baja_freq = fftshift(fft(demodulated_signal_baja));
filtro_pasabaja_baja = (abs(f_demodulated_signal) < Right);
demodulated_signal_baja_freq_filtrada = demodulated_signal_baja_freq .* filtro_pasabaja_baja;
demodulated_signal_baja_filtrada = ifft(ifftshift(demodulated_signal_baja_freq_filtrada));
...
demodulated_signal_media_freq = fftshift(fft(demodulated_signal_media));
filtro_pasabaja_media = (abs(f_demodulated_signal) < Left);
demodulated_signal_media_freq_filtrada = demodulated_signal_media_freq .* filtro_pasabaja_media;
demodulated_signal_media_filtrada = ifft(ifftshift(demodulated_signal_media_freq_filtrada));
...
demodulated_signal_alta_freq = fftshift(fft(demodulated_signal_alta));
filtro_pasabaja_alta = (abs(f_demodulated_signal) < Center);
demodulated_signal_alta_freq_filtrada = demodulated_signal_alta_freq .* filtro_pasabaja_alta;
demodulated_signal_alta_filtrada = ifft(ifftshift(demodulated_signal_alta_freq_filtrada));

% Graficar las señales demoduladas en el dominio del tiempo
figure;
subplot(3,1,1);
plot(t, demodulated_signal_baja_filtrada);
title('Señal Demodulada de la Subportadora Baja en el Dominio del Tiempo');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);

subplot(3,1,2);
plot(t, demodulated_signal_media_filtrada);
title('Señal Demodulada de la Subportadora Media en el Dominio del Tiempo');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);

subplot(3,1,3);
plot(t, demodulated_signal_alta_filtrada);
title('Señal Demodulada de la Subportadora Alta en el Dominio del Tiempo');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);

% Comparar la señal original y las señales demoduladas
figure;
subplot(4,1,1);
plot(t, fxRight);
title('Señal Original (fxRight)');
...
subplot(4,1,2);
plot(t, demodulated_signal_baja_filtrada);
title('Señal Demodulada (fxbajadem)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);
...
subplot(4,1,3);
plot(t, demodulated_signal_media_filtrada);
title('Señal Demodulada (fxmediadem)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);
...
subplot(4,1,4);
plot(t, demodulated_signal_alta_filtrada);
title('Señal Demodulada (fxaltadem)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0, 0.01]);

disp('Demodulación completada.');
