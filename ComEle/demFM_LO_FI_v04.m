...
% Frecuencia central para el filtro
band_start = 10.8e6;
band_end = 11.2e6;
...
bp_filter = (f_IF >= band_start & f_IF <= band_end);
...
% Graficar la señal filtrada con normalización
figure;
normalized_magnitude = abs(IF_signal_freq_filtered) / max(abs(IF_signal_freq_filtered));
plot(f_IF, fftshift(normalized_magnitude));
title('Señal IF Filtrada en el Dominio de la Frecuencia (Filtro de 10.8 a 11.2 MHz)');
xlabel('Frecuencia (Hz)');
ylabel('Magnitud (normalizada)');
xlim([10.8e6, 11.2e6]); % Mostrar un rango más cerrado para la señal filtrada
ylim([0, 1]); % Ajustar la escala de magnitud para mejor visualización
